class GigsController < ApplicationController
  ...
  def create
    ...
    if @gig.save
      #  at this moment, just create 3 different pricings with different pricing_type
      @gig.pricings.create(Pricing.pricing_types.values.map { |x| { pricing_type: x } })
      redirect_to edit_gig_path(@gig, step: 2), notice: "Save..."
    else
  ...
  def update
    if @step == 2
      gig_params[:pricings_attributes].each do |index, pricing|
        if @gig.has_single_pricing && pricing[:pricing_type] != Pricing.pricing_types.key(0) # Basic pricing type
          next
        else
  ...
  def is_authorized
    redirect_to root_path,
                alert: 'You do not have permission' unless current_user.id == @gig.user_id
  end

  def gig_params
    params.require(:gig).permit(
      :title,:description, :video, :active, :category_id, :has_single_pricing,
